...
积极建立、强化和实施普遍存在的领域语言 (CONTEXT.md) 和架构决策记录 (docs/adr/*.md)，以防止代理和工程团队之间出现语义漂移。

---

## 核心不变量

1. **主动语义执行**：主动挑战超载、模棱两可或口语化的术语，并实时将其与规范定义保持一致。
2. **立即内联术语表更新**：在领域术语具体化时将其捕获到“CONTEXT.md”中；切勿在会话结束时对术语表进行批量编辑。
3. **严格的无实现术语**：`CONTEXT.md`必须包含零实现细节、框架或数据库选择——它是一个纯领域字典。
...
|神器|责任|格式参考|
|---|---|---|
| **领域术语** |规范术语、实体边界、不变量 | `技能/领域建模/CONTEXT-FORMAT.md` |
| **建筑实录** |不可逆转的架构选择和权衡| `技能/域建模/ADR-FORMAT.md` |
| **上下文地图** |跨模块化存储库的有界上下文 | `上下文地图.md` |
