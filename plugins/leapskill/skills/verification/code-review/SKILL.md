---
name: code-review
description: "Review changed code against repository coding standards and original specification intent. Use when reviewing a branch, diff, PR, pull request, merge-base changes, or verifying code against documented standards — even if the user just says \"review this\". 用于用户要求审查这个分支、diff、PR 或改动是否符合编码标准和原始需求时。Do NOT use for authoring new code or diagnosing failing runtime bugs."
---
...
2. **规范**：代码是否忠实且完整地实现了原始problem/规范，而没有范围蔓延？
...
1. **严格的两轴分离**：将标准和规范结果分开；切勿将它们合并或交叉排列成混合评分。
...
|组件|责任|评测来源|
|---|---|---|
| **标准轴** |架构坏味道、命名、凝聚力 | `CODING_STANDARDS.md`、`CONTRIBUTING.md`、气味基线 |
...
### 第 1 步：固定固定点和差异
- **操作**：解析基准引用并确认非空差异（`git diff <base>...HEAD`）。
...
| *“它按照工单的要求进行操作，因此请忽略杂乱的架构。”* | **规格通过 $\neq$ 标准通过。** |绕过标准的临时黑客式实现会产生严重的技术债务。 |
