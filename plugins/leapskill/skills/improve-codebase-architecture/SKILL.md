...
| **HTML 可视化报告** |并排渲染 Mermaid 和 Tailwind 卡 ​​| `技能/改进代码库架构/HTML-REPORT.md` |
| **候选人深化** |对选定候选人的苏格拉底式审查| `技能/烧烤/SKILL.md` + `技能/代码库设计/SKILL.md` |
...
- **要点**：包括问题描述、杠杆/局部性优势以及美人鱼图之前/之后的并排。
- **为什么**：视觉架构图比文字墙更有效地传达结构改进。

### 第 4 步：打开报告并促进候选人选择
- **操作**：通过 `open <path>` (macOS) / `xdg-open` (Linux) / `start` (Windows) 启动 HTML 报告，并询问用户要探索哪个候选。
- **要点**：选择后，进入烧烤循环以解决模块边界并更新“CONTEXT.md”/ADR。
- **为什么**：协作候选人选择可确保团队在投资重构之前获得支持。
