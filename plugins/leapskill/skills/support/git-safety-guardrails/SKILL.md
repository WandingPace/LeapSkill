...
description: "Safeguard repositories against destructive, irreversible, or history-rewriting Git operations. Use when running force pushes, hard resets, branch deletions, cleans, destructive restores, or history re-writes — even if the user says \"clean up git history\". 用于用户要求拦截 force push、hard reset 等危险 Git 操作，或在清理或重写 Git 历史前加安全护栏时。Do NOT use for routine safe git status, fetch, diff, or branch queries."
---

# Git 安全护栏

安装和维护确定性的预执行护栏，在自主智能体执行危险、破坏性或历史重写的 Git 命令之前拦截和阻止这些命令。
...
4. **设置合并安全**：将护栏钩子无缝合并到现有的“PreToolUse”配置中，而不会覆盖其他工具或设置。
...
|组件|责任|地点 |
|---|---|---|
| **分类器钩子脚本** |检查并阻止禁止的 git 模式 | `scripts/block-dangerous-git.sh` |
...
| *“跳过使用模拟输入验证钩子脚本。”* | **强制模拟测试通过。** |钩子脚本中的语法错误会导致它们fail-open，使仓库失去保护，从而使存储库不受保护。 |
