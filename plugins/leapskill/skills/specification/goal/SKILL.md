---
name: goal
description: "Design and synthesize high-leverage autonomous goal prompts and contracts for unattended execution. Use when crafting a /goal prompt, planning an overnight autonomous coding run, converting rambling specifications into a self-contained agentic mission, or setting up multi-agent autonomous loops — even if they don't explicitly say \"goal prompt\". 用于用户要求写 /goal 提示词、安排通宵自主编码任务或设计多智能体目标合约时。Do NOT use when the user asks for immediate single-turn execution in the current conversation without an autonomous goal contract."
---

# 自主目标和合约合成

将用户意图、模糊要求或杂乱的规范转化为特殊的、独立的 `/goal` 提示，为无人值守、自主多轮执行做好准备。

## 核心原则

> **自主目标提示指定目的地、质量门槛和验证循环 - 绝不是事无巨细的分步脚本。**

---

## 核心不变量

1. **可观察的完成条件**：每个可交付成果必须拥有自主会话可以验证自身的具体完成状态（例如，测试套件通过、CLI 能在给定测试夹具上运行、构建输出存在）。
2. **前期授权和创作自由**：授予明确的权限以做出设计决策、选择内部工作流并解决歧义，而无需打扰用户。
3. **已验证的资源清单**：仅列出已针对实时环境进行验证的 2-4 个工具或路径。与明确的发现授权相结合。
4. **强制多遍验证**：强制要求至少 3 次与目标介质匹配的不同迭代遍。
5. **终局目标句**：以一个包含可交付成果和自主指令的放在末尾强化最终指令的句子结尾（“...是你的 /goal。完全自主地工作，在完成之前不要向我询问任何事情。”）。
...
| *“我在终局目标句后添加了注释和解释。”* | **终点线必须是最后一句话。** |新近度偏差确保最终指令锚定模型的主要目标。 |
