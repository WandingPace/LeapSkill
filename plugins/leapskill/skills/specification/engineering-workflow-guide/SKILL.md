...
|路由器参考|责任|文件位置 |
|---|---|---|
| **技能目录** |所有 42 个打包技能的详尽索引 | [references/catalog.md](references/catalog.md) |
| **ChatGPT 路由合约** |运行时调用和抗幻觉协议| [references/chatgpt-routing-contract.md](references/chatgpt-routing-contract.md) |
| **路由矩阵** | 100 多个意图模式、触发器和排除项 | [references/routing-matrix.md](references/routing-matrix.md) |
| **阶段边界** | `/clear`、`/compact`、`/handoff` 和子代理的决策树 | [PHASE-BOUNDARIES.md](PHASE-BOUNDARIES.md) |

|生命周期场景 |规范路由管道 |
|---|---|
| **模糊绿地特征** | `grill-with-docs` $\rightarrow$ `to-spec` $\rightarrow$ `to-tickets` $\rightarrow$ `implement` |
| **疑难 Bug或回归** | `diagnosing-bugs` $\rightarrow$ `tdd` |
| **拉取请求审查** | `code-review`（两轴：标准和规范）|
| **深度模块重组** | `improve-codebase-architecture` $\rightarrow$ `setup-ts-deep-modules` |
| **大型多会话地图** | `wayfinder` $\rightarrow$ `to-spec` $\rightarrow$ `to-tickets` |
| **深度多步骤推理** | `j-space`（认知寄存器和接缝审核）|
| **自主无人值守目标** | `goal`（7 部分即时合同）|
| **生产 ML / LLM 系统** | `ai-engineering` $\rightarrow$ `ml-best-practices` |
| **自我修复数据管道** | `ai-data-remediation` |
| **长篇写作和散文** | `writing-fragments` $\rightarrow$ `writing-shape` / `writing-beats` |
...
| *“发明一个不在目录中的新自定义工作流名称。”* | **专门路由到“catalog.md”中的打包技能。** |路由到不存在的技能会导致执行失败。 |
