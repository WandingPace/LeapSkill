...
description: "Interview the user to stress-test a design while simultaneously recording domain terms and architectural decisions in project documentation. Use when planning features in a codebase and wanting domain terms captured in CONTEXT.md and ADRs — even if the user says \"grill this feature\". 中文用户常在说“拷问这个设计，并把领域术语和架构决策记入文档”时请求此技能。 Do NOT use when no codebase context or documentation trail is needed."
---
...
1. **同步文档蒸馏**：随着决策的确定，将通用领域语言提取并记录到“CONTEXT.md”中，并将架构选择实时记录到 ADR 中。
...
5. **不写入推测性内容**：仅记录访谈期间已积极解决的决定；保持草稿笔记的清晰划分。
...
|产物|目的|参考格式|
|---|---|---|
| **领域词典** |捕获通用语言和实体关系 | `CONTEXT-FORMAT.md` |
| **架构决策记录** |记录不可逆的架构选择 | `ADR-FORMAT.md` |
| **拷问协议** |推动结构化前沿问题轮次| `skills/grilling/SKILL.md` |
