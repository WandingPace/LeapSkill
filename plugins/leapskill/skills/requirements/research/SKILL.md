---
name: research
description: "Investigate a technical question against high-trust primary sources and capture findings as a cited Markdown note. Use when gathering API facts, reading documentation, verifying library capabilities, or delegating reading legwork — even if the user says \"look into this library\". 中文用户常在说“调研这个库”“查一下官方文档”或“研究这个技术方案”时请求此技能。 Do NOT use for writing production feature code."
---
...
1. **仅使用主要来源**：以官方文档、源代码、正式规范或第一方发行说明中的​​所有发现为依据；永远不要依赖未经验证的博客文章或二次摘要。
2. **强制标明出处并逐条引用**：每个技术声明、API 签名或版本约束都必须直接链接到其主要源 URI 或存储库文件路径。
...
- **关键点**：区分硬技术限制（例如速率限制、内存占用）和易用性权衡。
- **为什么**：无限制的研究会导致过度的token 消耗，而没有回答核心决策问题。
...
|诱人的合理化|约束规则|工程原理 |
|---|---|---|
| *“我记得这个库是如何根据训练数据工作的，因此无需查找当前文档。”* | **所有库事实的强制主要来源查找。** |训练记忆会产生已弃用的 API 签名的幻觉，并错过最近的重大更改。 |
