...
对过去的代理编码会话进行系统的、基于证据的回顾，以改进存储库导航指针、自动 lint/类型检查、审阅者标准和指令人体工程学。
...
3. **空操作指令修剪**：主动审核并消除`AGENTS.md`和`CLAUDE.md`中的非操作或冗余指令。
4. **工具经济分析**：识别并简化令牌效率低下的 MCP 工具调用或大文件负载读取。
5. **按严重程度排名的建议**：提出可操作的改进候选方案，严格按照对代理可靠性和代币效率的影响排序。
...
| **转向卫生** |笨重的 `AGENTS.md` / `CLAUDE.md` |修剪无操作、修剪散文、将规则推送到子文档 |
...
- **为什么**：回顾必须解决实际执行中观察到的真正的开发人员和代理摩擦。
...
- **内嵌清单**：
  ...
  - [ ] `AGENTS.md` 文件是否精简（$<100$ 行）并且没有任何操作？
...
| *“保留无操作指令，因为它们听起来不错。”* | **修剪所有不能明显引导模型行为的指令。** |死指令会消耗令牌并削弱对关键不变量的关注。 |
